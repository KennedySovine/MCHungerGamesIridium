...
public class HgCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (matchedKey == null) {
            MessageUtils.send(sender, "&cUnknown subcommand: " + args[0]);
            //sendHelp(sender);
            return true;
        }
        ...
        if (cmd == null) {
            MessageUtils.send(sender, "&cUnknown subcommand: " + matchedKey);
            return true;
        }
        ...
        try {
            ...
            ex.printStackTrace();
            MessageUtils.send(sender, "&cAn internal error occurred while executing the command.");
            return true;
        }
    }
    ...
    private void sendHelp(CommandSender sender) {
        ...
        for (Map.Entry<String, SubCommand> e : commands.entrySet()) {
            ...
            if (node != null && !node.isEmpty() && !sender.hasPermission(node)) continue;
            MessageUtils.send(sender, "&e/hg " + e.getKey() + " &7- " + sc.usage());
        }
    }
}
